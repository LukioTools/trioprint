...
"etElementById(\'fileInput\').click()\">Select File</button><div class=\"status-text\" id=\"statusText\"></d"
"iv></div><div class=\"restart-container\"><button class=\"restart-button\" onclick=\"reboot()\">Reboot</bu"
"tton><div class=\"\" id=\"rebootStatusText\"></div></div><div id=\"status\"></div></div><script>const conf"
"igMap = {wifi_ssid: 0,wifi_pwd: 1,ota_pwd: 2,web_name: 3,web_server_port: 4,web_socket_port: 5,sd_se"
"ctor_size: 6,file_chunk_size: 7,sd_spi_speed: 8,sd_card_select_pin: 9,sd_card_max_attempts: 10,dev_s"
"erial_group: 11,deb_serial_group: 12,printer_buffer_size: 13,printer_command_size: 14,printer_timeou"
"t: 15}; const inputs = document.querySelectorAll(\'input, select\');async function saveField(input) {c"
"onst id = input.id;statusDiv = input.nextElementSibling;let configId;const formData = new FormData()"
";if (id.startsWith(\'dev_\')) {configId = configMap.dev_serial_group;formData.append(\'config\', configI"
"d);formData.append(\'br\', document.getElementById(\'dev_baudrate\').value);formData.append(\'c\', parseIn"
"t(document.getElementById(\'dev_config\').value, 10));formData.append(\'s\', document.getElementById(\'de"
"v_serial\').value);formData.append(\'rx\', document.getElementById(\'dev_rx\').value);formData.append(\'tx"
"\', document.getElementById(\'dev_tx\').value);formData.append(\'cm\', document.getElementById(\'dev_custo"
"m\').value);} else if (id.startsWith(\'deb_\')) {configId = configMap.deb_serial_group;formData.append("
"\'config\', configId);formData.append(\'br\', document.getElementById(\'deb_baudrate\').value);formData.ap"
"pend(\'c\', parseInt(document.getElementById(\'deb_config\').value, 10));formData.append(\'s\', document.g"
"etElementById(\'deb_serial\').value);formData.append(\'rx\', document.getElementById(\'deb_rx\').value);fo"
"rmData.append(\'tx\', document.getElementById(\'deb_tx\').value);formData.append(\'cm\', document.getEleme"
"ntById(\'deb_custom\').value);formData.append(\'e\', document.getElementById(\'deb_enabled\').value);} els"
"e {configId = configMap[id];if (configId === undefined) {console.warn(`No mapping for ${id}`);return"
";}formData.append(\'config\', configId);formData.append(\'status\', input.value);}statusDiv.innerHTML = "
"`Saving ${id}...`;statusDiv.style.color = \'black\';try {const response = await fetch(\'/config/setDyna"
"micConfig\', {method: \'POST\',body: formData});if (!response.ok) throw new Error(`HTTP ${response.stat"
"us}`);const text = await response.text();if (text !== \'saved\') throw new Error(`Server error: ${text"
"}`);statusDiv.innerHTML = `Saved ${id} successfully.`;statusDiv.style.color = \'green\';} catch (err) "
"{statusDiv.innerHTML = `Error saving ${id}: ${err.message}`;statusDiv.style.color = \'red\';}} inputs."
"forEach(input => {input.addEventListener(\'change\', () => {if (input.required && input.value.trim() ="
"== \'\') {input.style.borderColor = \'red\';return;} else {input.style.borderColor = \'#ccc\';}saveField(i"
"nput);});}); const devCustomSelect = document.getElementById(\'dev_custom\');const debCustomSelect = d"
"ocument.getElementById(\'deb_custom\');const devCustomFields = document.getElementById(\'dev_custom_fie"
"lds\');const debCustomFields = document.getElementById(\'deb_custom_fields\');devCustomSelect.addEventL"
"istener(\'change\', function () {devCustomFields.classList.toggle(\'hidden\', this.value !== \'1\');});deb"
"CustomSelect.addEventListener(\'change\', function () {debCustomFields.classList.toggle(\'hidden\', this"
".value !== \'1\');}); devCustomFields.classList.toggle(\'hidden\', devCustomSelect.value !== \'1\');debCus"
"tomFields.classList.toggle(\'hidden\', debCustomSelect.value !== \'1\'); function updateProgress(progres"
"s, status) {const progressBar = document.getElementById(\'progressBar\');const statusText = document.g"
"etElementById(\'statusText\');progressBar.style.width = progress + \"%\";statusText.innerText = `${statu"
"s}: ${progress}%`;} document.getElementById(\'fileInput\').addEventListener(\'change\', function (event)"
" {const file = event.target.files[0];if (file) {document.getElementById(\'progressContainer\').style.d"
"isplay = \'block\';upload_file(file);}}); function xhrFetch(url, options) {options = options || {};ret"
"urn new Promise(function (resolve, reject) {var xhr = new XMLHttpRequest(); xhr.open(options.method "
"|| \'GET\', url); if (options.headers) {for (var key in options.headers) {if (options.headers.hasOwnPr"
"operty(key)) {xhr.setRequestHeader(key, options.headers[key]);}}} if (options.responseType) {xhr.res"
"ponseType = options.responseType;} if (xhr.upload && options.onProgress) {xhr.upload.onprogress = fu"
"nction (event) {if (event.lengthComputable) {const progress = (event.loaded / event.total) * 100;opt"
"ions.onProgress(progress, event.loaded, event.total);}};} xhr.onload = function () {if (xhr.status >"
"= 200 && xhr.status < 300) {resolve({ok: true,status: xhr.status,statusText: xhr.statusText,json: fu"
"nction () {return Promise.resolve(JSON.parse(xhr.responseText));},text: function () {return Promise."
"resolve(xhr.responseText);}});} else {reject(new Error(\'HTTP Error: \' + xhr.status));}}; xhr.onerror"
" = function () {reject(new Error(\'Network Error\'));}; xhr.send(options.body || null);});} function u"
"pload_file(file) {cwd = \'/\';console.log(`uploading file: \'${file.name}\' to path \'${cwd}\'`);const for"
"mData = new FormData();formData.append(\"file\", file);xhrFetch(`/fm/uploadFile/?path=${cwd}`, {method"
": \"POST\",body: formData,onProgress: function (progress, loaded, total) {console.log(`Progress: ${pro"
"gress.toFixed(2)}% (${loaded}/${total} bytes)`);updateProgress(progress.toFixed(2), \"UPLOAD STATUS\")"
";}}).then(function (respz) {console.log(respz);files();}).catch(function (err) {console.error(err);c"
"onst statusDiv = document.getElementById(\'statusText\');statusDiv.innerHTML = `Error uploading ${file"
".name}: ${err.message}`;statusDiv.style.color = \'red\';});} async function fetchSDCardStatus() {try {"
"const response = await fetch(\'/server/sdCardStatus\');const data = await response.json();const sdCard"
"StatusElement = document.getElementById(\'SdCardStatus\'); if (data.cardStatus) {sdCardStatusElement.t"
"extContent = \'SD Card is Initialized\';sdCardStatusElement.classList.remove(\'error\');sdCardStatusElem"
"ent.classList.add(\'success\');} else {sdCardStatusElement.textContent = \'SD Card is NOT Initialized\';"
"sdCardStatusElement.classList.remove(\'success\');sdCardStatusElement.classList.add(\'error\');}} catch "
"(error) {document.getElementById(\'SdCardStatus\').textContent = \'Error fetching status\';document.getE"
"lementById(\'SdCardStatus\').classList.add(\'error\');}} fetchSDCardStatus(); setInterval(fetchSDCardSta"
"tus, 500); async function fetchFlashStorage() {for (const [id, configNum] of Object.entries(configMa"
"p)) {try {const response = await fetch(`/config/getDynamicConfig?config=${configNum}`);const text = "
"await response.text();const element = document.getElementById(id);if (element) {element.value = text"
".trim();}} catch (error) {console.error(`Failed to fetch config ${configNum} for ${id}`, error);}} t"
"ry {const devResponse = await fetch(`/config/getDynamicConfig?config=11`);devText = await devRespons"
"e.text();[baudRate, config, serial, rx, tx, custom] = devText.split(\',\'); document.getElementById(\'d"
"ev_baudrate\').value = baudRate;document.getElementById(\'dev_serial\').value = serial;document.getElem"
"entById(\'dev_custom\').value = custom; devCustomFields.classList.toggle(\'hidden\', true); if (custom ="
"== \"true\") {devCustomFields.classList.toggle(\'hidden\', false);document.getElementById(\'dev_custom_fi"
"elds\').classList.remove(\'hidden\');document.getElementById(\'dev_config\').value = config;document.getE"
"lementById(\'dev_rx\').value = rx;document.getElementById(\'dev_tx\').value = tx;}} catch (error) {conso"
"le.error(\'Failed to fetch DevSerialConfig\', error);} try {const debResponse = await fetch(`/config/g"
"etDynamicConfig?config=12`);debText = await debResponse.text();[baudRate, config, serial, rx, tx, cu"
"stom, enabled] = debText.split(\',\'); document.getElementById(\'deb_baudrate\').value = baudRate;docume"
"nt.getElementById(\'deb_serial\').value = serial;document.getElementById(\'deb_custom\').value = custom;"
"document.getElementById(\'deb_enabled\').value = enabled; console.log(serial); debCustomFields.classLi"
"st.toggle(\'hidden\', true); if (custom === \"true\") {debCustomFields.classList.toggle(\'hidden\', false)"
";document.getElementById(\'deb_custom_fields\').classList.remove(\'hidden\');document.getElementById(\'de"
"b_config\').value = config;document.getElementById(\'deb_rx\').value = rx;document.getElementById(\'deb_"
"tx\').value = tx;}} catch (error) {console.error(\'Failed to fetch DebugSerialConfig\', error);}} windo"
"w.addEventListener(\'DOMContentLoaded\', fetchFlashStorage); async function reboot() {try {const respo"
"nse = await fetch(\'/server/reboot\');const data = await response.json();const rebootStatusElement = d"
"ocument.getElementById(\'rebootStatusText\'); if (data.cardStatus) {rebootStatusElement.textContent = "
"\'reboot successful\';rebootStatusElement.classList.remove(\'error\');location.reload()} else {rebootSta"
"tusElement.textContent = \'reboot failed\';rebootStatusElement.classList.add(\'error\');}} catch (error)"
" {document.getElementById(\'rebootStatusText\').textContent = \'Error fetching status\';document.getElem"
"entById(\'rebootStatusText\').classList.add(\'error\');}}</script></body></html>"
;
